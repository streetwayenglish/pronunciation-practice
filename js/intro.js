...
(function(){
  ...
  /* ---------- SPLASH (cover layer) ---------- */
  + '.es-splash{position:absolute;inset:0;z-index:2;background:#F1EFE6;cursor:pointer;'
  +   'user-select:none;-webkit-user-select:none;overflow:hidden;'
  +   'transition:opacity .7s ease,visibility .7s;opacity:1;visibility:visible;}'
  + '.es-splash.off{opacity:0;visibility:hidden;pointer-events:none;}'
  + '.es-blob{position:absolute;border-radius:50%;filter:blur(24px);'
  +   'background:radial-gradient(circle,rgba(246,183,40,0.55) 0%,rgba(246,183,40,0) 70%);}'
  + '.es-blob.b1{left:-160px;top:-60px;width:360px;height:360px;opacity:.55;'
  +   'animation:es-drift 19s ease-in-out infinite alternate;}'
  + '.es-blob.b2{left:240px;top:600px;width:320px;height:320px;opacity:.45;'
  +   'animation:es-drift2 24s ease-in-out infinite alternate;}'
  + '.es-lockup{position:absolute;left:50%;top:48%;width:300px;height:53.5px;'
  +   'transform:translate(-50%,-50%);}'
  + '.es-word{position:absolute;inset:0;clip-path:inset(-12% 50% -12% 50%);'
  +   'animation:es-reveal 1.15s cubic-bezier(0.76,0,0.24,1) 0.85s forwards;}'
  + '.es-word img{width:300px;height:53.5px;display:block;-webkit-user-drag:none;}'
  + '.es-dot{position:absolute;left:300.65px;top:35.75px;width:7.1px;height:7.1px;'
  +   'transform:translate(-154.2px,-12.55px);'
  +   'animation:es-travel 1.15s cubic-bezier(0.76,0,0.24,1) 0.85s forwards;}'
  + '.es-dot i{display:block;width:100%;height:100%;border-radius:50%;background:#F6B728;'
  +   'transform:scale(0);'
  ...
  /* ---------- ONBOARDING ---------- */
  + '.em-onb{position:absolute;inset:0;display:flex;flex-direction:column;background:#FAF6EE;'
  ...
  + '.em-onb.on{opacity:1;}'
  + '.em-onb-inner{max-width:430px;width:100%;margin:0 auto;display:flex;flex-direction:column;'
  +   'min-height:100%;}'
  + '.em-dashes{display:flex;gap:10px;justify-content:center;padding:8px 0 0;}'
  + '.em-dash{width:26px;height:4px;border-radius:99px;background:#E4DCCB;}'
  + '.em-dash.on{width:34px;background:#EDB111;}'
  + '.em-logo{display:flex;justify-content:center;margin-top:20px;}'
  + '.em-logo img{height:26px;width:auto;display:block;}'
  + '.em-rise{animation:em-rise 0.7s cubic-bezier(0.22,1,0.36,1) 0.2s both;}'
  + '.em-lead{margin:26px 0 0;font-size:16px;color:rgba(43,38,32,.6);}'
  + '.em-h1{font-family:"Source Serif 4",Georgia,serif;font-size:30px;line-height:1.1;'
  +   'font-weight:700;letter-spacing:-.01em;margin:2px 0 0;}'
  + '.em-goals{display:flex;flex-direction:column;gap:12px;margin-top:20px;}'
  + '.em-goal{background:#FFFFFF;border:1.5px solid #EDE6D5;border-radius:20px;'
  ...
  + '.em-goal:active{transform:scale(.985);}'
  + '.em-goal.sel{background:#FCF3DC;border-color:#EDB111;}'
  + '.em-tile{width:56px;height:56px;border-radius:16px;flex:none;overflow:hidden;position:relative;}'
  + '.em-tile img{position:absolute;max-width:none;transform:translate(-50%,-50%);'
  +   '-webkit-user-drag:none;}'
  + '.em-goal-txt{flex:1;display:flex;flex-direction:column;gap:2px;min-width:0;}'
  + '.em-goal-lb{font-weight:700;font-size:16.5px;letter-spacing:-.01em;}'
  + '.em-goal-hint{font-size:13px;color:rgba(43,38,32,.5);}'
  + '.em-check{width:26px;height:26px;border-radius:50%;background:transparent;'
  ...
  + '.em-goal.sel .em-check{background:#EDB111;border-color:#EDB111;}'
  + '.em-spacer{flex:1;min-height:18px;}'
  + '.em-name-lb{margin:0 0 8px;font-size:14px;color:rgba(43,38,32,.6);}'
  + '.em-name-row{display:flex;align-items:center;background:#fff;border:1.5px solid #E9E1CF;'
  +   'border-radius:16px;padding:0 16px;}'
  + '.em-name-row input{border:none;background:transparent;padding:15px 0;font-size:17px;'
  +   'font-weight:600;outline:none;color:#2B2620;flex:1;font-family:"Bricolage Grotesque",system-ui,sans-serif;min-width:0;}'
  + '.em-name-row input::placeholder{color:#C0B8A6;}'
  + '.em-cta{background:#F0C550;color:#3A3006;border:0;border-radius:999px;padding:17px 0;'
  ...
  + '.em-cta.ready{background:#EDB111;}'
  + '.em-cta:active{transform:scale(.98);}'
  + '.em-skip{text-align:center;font-size:13.5px;color:rgba(43,38,32,.5);margin-top:12px;'
